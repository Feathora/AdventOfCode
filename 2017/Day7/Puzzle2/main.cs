...
namespace Day7
{
    class Day7
    {
        private const string input = @"mypyemv (1058) -> tdssotr, pebnvks, zaulju
snslv (48)
aytbgf (30)
mkcrsy (42)
shwcd (42)
...
tnmtz (26)";
        ...
        class Program
        {
            ...
            public int Weight
            {
                get
                {
                    return weight + (from program in programs select program.Weight).Sum();
                }
            }
        }

        static List<Program> allPrograms;
        static Program bottomProgram;

        static Program FindProgram(string name)
        {
        ...
        static Program FindOddOne(Program program)
        {
            Dictionary<int, List<Program>> d = new Dictionary<int, List<Program>>();
            foreach(var child in program.programs)
            {
                if(!d.ContainsKey(child.Weight)) d.Add(child.Weight, new List<Program>());
                
                d[child.Weight].Add(child);
            }
        ...
        static void CheckBalance(Program program)
        {
            var odd = FindOddOne(program);

            if(odd == null)
            {
                int normalWeight = (from p in program.parent.programs where p != program select p).First().Weight;
                int weightDiff = program.Weight - normalWeight;

                int neededWeight = program.weight - weightDiff;
        ...
        static void Main(string[] args)
        {
            ...
            using(var reader = new StringReader(input))
            {
                ...
                while((line = reader.ReadLine()) != null)
                {
                    ...
                    var match = Regex.Match(line, @"\((\d+)\)").Groups[1].Value;

                    var programNames = new List<string>();

                    var words = Regex.Matches(line, @"\w(?<!\d)[\w'-]*");
                    for(int i = 1; i < words.Count; ++i)
                    {
                        programNames.Add(words[i].Value);
                    }

                    var program = new Program { name = name, weight = int.Parse(match), programNames = programNames };
            ...
            foreach(var program in allPrograms)
            {
                program.programs = new List<Program>();

                foreach(var childName in program.programNames)
                {
                    var child = FindProgram(childName);
                    program.programs.Add(child);
