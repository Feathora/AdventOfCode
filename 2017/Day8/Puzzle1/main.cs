...
namespace Day8
{
    class Day8
    {
        ...
        static Dictionary<string, Func<int, int, bool>> checks;
        static Dictionary<string, Action<string, int>> operations;

        const string input = @"v inc 523 if t == 6
...
ml inc 52 if lb > 1564";
        ...
        {
            Func<int, int, bool> eq = (lhs, rhs) => lhs == rhs;
            Func<int, int, bool> neq = (lhs, rhs) => lhs != rhs;
            Func<int, int, bool> l = (lhs, rhs) => lhs < rhs;
            Func<int, int, bool> m = (lhs, rhs) => lhs > rhs;
            Func<int, int, bool> leq = (lhs, rhs) => lhs <= rhs;
            Func<int, int, bool> meq = (lhs, rhs) => lhs >= rhs;

            checks = new Dictionary<string, Func<int, int, bool>>();
            checks.Add("==", eq);
            checks.Add("!=", neq);
            checks.Add("<", l);
            checks.Add(">", m);
            ...
            Action<string, int> inc = (register, amount) => registers[register] += amount;
            Action<string, int> dec = (register, amount) => registers[register] -= amount;

            operations = new Dictionary<string, Action<string, int>>();
            operations.Add("inc", inc);
            operations.Add("dec", dec);
            ...
            using(var reader = new StringReader(input))
            {
                ...
                while((line = reader.ReadLine()) != null)
                {
                    ...
                    string check = words[5];
                    int checkAgainst = int.Parse(words[6]);
                    ...
                    if(checks[check](registers[checkRegister], checkAgainst))
                    {
